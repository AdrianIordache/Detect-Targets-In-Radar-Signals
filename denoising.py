...
def run(GPU, CFG, PATH_TO_MODELS, logger):
    seed_everything(SEED)

    DEVICE = torch.device('cuda:{}'.format(GPU) if torch.cuda.is_available() else 'cpu')
    train  = pd.read_csv(PATH_TO_DENOISING)
...
if __name__ == "__main__":
    ...
    CFG = {
        ...
        'model_name': 'tf_efficientnet_b0', # 'beit_large_patch16_224_in22k', # 'swin_large_patch4_window12_384_in22k',
        'dropout': 0.5,
        'size': 384,
        'batch_size_t': 5,
        'batch_size_v': 12,
    ...
    PATH_TO_MODELS = 'data/detect-targets-in-radar-signals/denoising/models/'
    if SAVE_TO_LOG:
    ...
    accuracy = run(GPU, CFG, PATH_TO_MODELS, logger)
